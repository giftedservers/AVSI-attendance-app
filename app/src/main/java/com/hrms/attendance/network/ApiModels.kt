package com.hrms.attendance.network
...
data class TsLeaveType(
    val key: String,
    val label: String
)

/** Everything needed to render/submit a timesheet for one month, matching
 *  api/timesheet.php's `action=form_data` response. The saved/suggested
 *  hour maps come back as JSON objects keyed by day number as a string
 *  (e.g. {"14": 8}), so they're modeled as Map<String, Map<String, Double>>
 *  (outer key = project id or leave key, inner key = day-of-month). */
data class TimesheetFormData(
    val success: Boolean,
    val error: String? = null,
    ...
    val month: Int = 0,
    val year: Int = 0,
    @SerializedName("days_in_month") val daysInMonth: Int = 0,
    val status: String? = null,
    val location: String? = null,
    val title: String? = null,
    val projects: List<TsProject> = emptyList(),
    @SerializedName("leave_types") val leaveTypes: List<TsLeaveType> = emptyList(),
    @SerializedName("saved_project_hours") val savedProjectHours: Map<String, Map<String, Double>> = emptyMap(),
    @SerializedName("saved_leave_hours") val savedLeaveHours: Map<String, Map<String, Double>> = emptyMap(),
    @SerializedName("suggested_leave_hours") val suggestedLeaveHours: Map<String, Map<String, Double>> = emptyMap(),
    @SerializedName("public_holidays") val publicHolidays: Map<String, String> = emptyMap(),
    @SerializedName("blocked_days") val blockedDays: Map<String, String> = emptyMap()
)

/** One row in the "My Timesheets" list — matches api/timesheet.php's
 *  `action=my_timesheets` response. */
...
data class MyTimesheetsResponse(
    val success: Boolean,
    val error: String? = null,
    val timesheets: List<TimesheetSummary> = emptyList()
)

data class SubmitTimesheetResponse(
    val success: Boolean,
    val error: String? = null,
    val message: String? = null,
    @SerializedName("timesheet_id") val timesheetId: Int? = null,
    @SerializedName("total_hours") val totalHours: Double? = null
)
